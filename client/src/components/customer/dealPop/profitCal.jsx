...
const NetProfit = ({ deal, sector, currentValuation, userId }) => {
    ...
    return (
        <>
            <td className="table_cell" style={{ color: profit.toString() == "NaN" ? "black" : profit < 0 ? "red" : "green", fontSize: '12px' }} >
                {profit}
            </td>
            {/* <td>{sector}</td> */}
            <td className="table_cell" style={{ color: moic.toString() == "NaN" ? "black" : moic < 0 ? "red" : "green", fontSize: '12px' }}>
                {moic && moic?.toString()?.substring(0, 4) + 'x'}
            </td>
            <td className="table_cell" style={{ color: irr.toString() === "NaN" ? "black" : irr < 0 ? "red" : "green", fontSize: '12px' }}>
                {irr?.length > 10
                    ? `${irr.slice(0, 10)}...%`
                    : irr + "%"}
            </td>
